# 🚀 Developer Operations Cheatsheet

If you make changes to the **Frontend** (React) or **Backend** (FastAPI) code, here is the exact step-by-step process you need to follow to push those changes live to your Helm installation.

---

## Scenario 1: You updated FRONTEND or BACKEND Code

Whenever you change code in the `/frontend/src` or `/backend/api` folders, you need to build a new Docker image before you can update Helm.

### Step 1.1: Build and Push new Docker Images
*Ensure you are logged into Docker (`docker login`) first.*
...
### Step 1.2: Update the Helm `values.yaml` file
Open `charts/values.yaml` and update the `tag:` fields to match the version you just pushed (e.g., `"v3"`).

```yaml
api:
  image: ashrith2727/backend-metrics
  tag: "v3"  <-- CHANGE THIS

dashboard:
  image: ashrith2727/frontend-metrics
  tag: "v3"  <-- CHANGE THIS
```

### Step 1.3: Proceed to "Updating the Helm Chart" below. 👇
...
## 🔄 How Users Upgrade Their Clusters

Once you have pushed to `gh-pages`, anyone (including you) can pull the new code into their Kubernetes cluster running these two commands:
