...
from db.utils import log_action
...
def create_namespace(req: CreateNamespaceRequest, current_user: str = Depends(get_current_user), db: Session = Depends(get_db)):
    data = k8s.create_namespace(req.name)
    if isinstance(data, dict) and "error" in data:
        raise HTTPException(status_code=500, detail=data["error"])
    log_action(db, current_user, "CREATE", "Namespace", req.name)
    return data

@router.delete("/metrics/namespaces/{name}")
def delete_namespace(name: str, current_user: str = Depends(get_current_user), db: Session = Depends(get_db)):
    data = k8s.delete_namespace(name)
    if isinstance(data, dict) and "error" in data:
        raise HTTPException(status_code=500, detail=data["error"])
    log_action(db, current_user, "DELETE", "Namespace", name)
    return data
...
def delete_pod(namespace: str, pod_name: str, current_user: str = Depends(get_current_user), db: Session = Depends(get_db)):
    data = k8s.delete_pod(pod_name, namespace)
    if isinstance(data, dict) and "error" in data:
        raise HTTPException(status_code=500, detail=data["error"])
    log_action(db, current_user, "DELETE", "Pod", pod_name, namespace)
    return data

@router.post("/metrics/deployments/{namespace}/{deployment_name}/restart")
def restart_deployment(namespace: str, deployment_name: str, current_user: str = Depends(get_current_user), db: Session = Depends(get_db)):
    data = k8s.restart_deployment(deployment_name, namespace)
    if isinstance(data, dict) and "error" in data:
        raise HTTPException(status_code=500, detail=data["error"])
    log_action(db, current_user, "RESTART", "Deployment", deployment_name, namespace)
    return data

@router.post("/metrics/deployments/{namespace}/{deployment_name}/scale")
def scale_deployment(namespace: str, deployment_name: str, req: ScaleRequest, current_user: str = Depends(get_current_user), db: Session = Depends(get_db)):
    data = k8s.scale_deployment(deployment_name, req.replicas, namespace)
    if isinstance(data, dict) and "error" in data:
        raise HTTPException(status_code=500, detail=data["error"])
    log_action(db, current_user, "SCALE", "Deployment", deployment_name, namespace, f"Scaled to {req.replicas} replicas")
    return data
