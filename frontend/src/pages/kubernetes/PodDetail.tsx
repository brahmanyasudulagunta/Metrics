...
import {
    ...
    DialogContentText, DialogActions, Breadcrumbs, Link, TextField, Grid, Fade
} from '@mui/material';
    ...
    return (
        <Fade in={true}>
            <Box sx={{ display: 'flex', flexDirection: 'column', p: 3 }}>
                            ...
                            {isEditingPath ? (
                                <TextField
                                    ...
                                    onKeyDown={(e) => {
                                        if (e.key === 'Enter') {
                                            ...
                                            if (parts[0] === 'clusters') {
                                                if (parts.length === 1) navigate(`/dashboard/kubernetes${cluster ? `?cluster=${cluster}` : ''}`);
                                                else if (parts[1] === 'namespace') {
                                                    if (parts.length === 2) navigate(`/dashboard/kubernetes?ns=all&ot=0${cluster ? `&cluster=${cluster}` : ''}`);
                                                    else navigate(`/dashboard/kubernetes?ns=${parts[2]}&ot=1&rt=0${cluster ? `&cluster=${cluster}` : ''}`);
                                                }
                                            }
                                ...
                                >
                                    <Box
                                        ...
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            navigate(`/dashboard/kubernetes${cluster ? `?cluster=${cluster}` : ''}`);
                                        }}
                                    >
                                    ...
                                    <Box
                                        ...
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            navigate(`/dashboard/kubernetes?ns=all&ot=0${cluster ? `&cluster=${cluster}` : ''}`);
                                        }}
                                    >
                                    ...
                                    <Box
                                        ...
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            navigate(`/dashboard/kubernetes?ns=${namespace}&ot=1&rt=0${cluster ? `&cluster=${cluster}` : ''}`);
                                        }}
                                    >
                                        <Typography sx={{ fontSize: '0.85rem', color: '#c9d1d9' }}>{namespace}</Typography>
                                    </Box>
                ...
                <Box sx={{ width: '100%', display: 'flex', gap: 3, flexDirection: { xs: 'column', md: 'row' } }}>
                    ...
                    <Box sx={{ flex: '0 0 35%', minWidth: 300, display: 'flex', flexDirection: 'column', gap: 2 }}>
                        ...
                        {podInfo && podInfo.containers && podInfo.containers.length > 0 && (
                            <Paper sx={{ p: 2, bgcolor: tokens.bg.surface, borderRadius: 2 }}>
                                <Typography variant="h6" sx={{ mb: 2, fontSize: '1rem' }}>Containers</Typography>
                                <Box sx={{ pr: 1 }}>
                        ...
                        {podInfo && podInfo.labels && Object.keys(podInfo.labels).length > 0 && (
                            <Paper sx={{ p: 2, bgcolor: tokens.bg.surface, borderRadius: 2 }}>
                                <Typography variant="h6" sx={{ mb: 2, fontSize: '1rem' }}>Labels</Typography>
                    ...
                    {/* Right Column: Logs */}
                    <Paper sx={{ flex: 1, bgcolor: tokens.bg.base, display: 'flex', flexDirection: 'column', borderRadius: 2, overflow: 'hidden', height: 600 }}>
